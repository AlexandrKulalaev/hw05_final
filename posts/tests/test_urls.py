...
class PostURLTests(TestCase):
    ...
    def test_username_post_url_exists_at_desired_location(self):
        ...
        response = self.guest_client.get(f'/{PostURLTests.user_author}/{PostURLTests.post.id}/')
        self.assertEqual(response.status_code, 200)
    ...
    def test_edit_url_redirect_anonymous_on_admin_login(self):
        ...
        response = self.guest_client.get(f'/{PostURLTests.user_author}/{PostURLTests.post.id}/edit/', follow=True)
        self.assertRedirects(response, f'/auth/login/?next=/{PostURLTests.user_author}/{PostURLTests.post.id}/edit/') 

    def test_edit_url_redirect_auth_users(self):
        """Залогированному пользователю не доступно редактирование поста, если он не автор"""
        response = self.authorized_client_other.get(reverse("post_edit", args=[PostURLTests.user_author.username, 
            PostURLTests.post.id]), follow=True)
        self.assertRedirects(response, f'/{PostURLTests.user_author}/{PostURLTests.post.id}/')

    def test_auth_author_user_page_edit(self):
        """Автору поста доступно редактирование поста"""
        response = self.authorized_client_author.get(reverse("post_edit", args=[PostURLTests.user_author.username, 
